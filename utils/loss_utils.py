...
from utils.box_utils import bbox_iou, xywh2xyxy, generalized_box_iou, distance_box_iou, complete_box_iou
...
def mytrans_vg_loss(batch_pred, batch_target):
    """Compute the losses related to the bounding boxes, 
       including the L1 regression loss and the CIoU loss
    """
    batch_size = batch_pred.shape[0]
    # world_size = get_world_size()
    num_boxes = batch_size
    ...
    loss_bbox = F.l1_loss(batch_pred, batch_target, reduction='none')
    loss_diou = 1 - torch.diag(distance_box_iou(
        xywh2xyxy(batch_pred),
        xywh2xyxy(batch_target)
    ))
...
def ciou_loss(batch_pred, batch_target):
    """Compute the losses related to the bounding boxes, 
       including the L1 regression loss and the CIoU loss
    """
    batch_size = batch_pred.shape[0]

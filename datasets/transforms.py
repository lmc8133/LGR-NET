...
def bbox_recover(bbox, origin_imgsize, imsize=640):
    for i, box in enumerate(bbox):  #box[x_c,y_c,w,h]
        bbox[i]=bbox[i]*imsize
        w, h=origin_imgsize[i]
        ratio=float(imsize/(float(max(h, w))))
        dh=float(imsize-h*ratio)
        dw=float(imsize-w*ratio)
        top = round(dh / 2.0 - 0.1)
        left = round(dw / 2.0 - 0.1)
        bbox[i][0], bbox[i][2] = bbox[i][0]-left, bbox[i][2]-left
        bbox[i][1], bbox[i][3] = bbox[i][1]-top, bbox[i][3]-top

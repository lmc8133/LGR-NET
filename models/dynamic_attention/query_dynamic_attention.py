...
class ChannelGate(nn.Module):
    ...
    def forward(self, x ,mu):
        ...
        for pool_type in self.pool_types:
            ...
            if pool_type=='avg':
                avg_pool=torch.mean(pre_pool,dim=1) #[B, D](8,96)
            ...
            if channel_att_sum is None:
                ...
                channel_att_sum = channel_att_sum + channel_att_raw #[B,D]
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x, mu):
        ...
        x_out = self.spatial(x,mu)  #[B,(H*W),1]
...
class QueryDynamicAttention(nn.Module):
    ...
    def forward(self, x,mu):
        if self.use_channel:
            x = self.ChannelGate(x,mu)  #[B,(H*W),C](8,25600,96)
        if len(x.size())<=2:
            return x
        if self.use_spatial:
